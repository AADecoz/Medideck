            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex items-center justify-between p-6 bg-white border-b border-gray-200">
                    ...
                    @if(Auth::user()->hasRole('administrator'))
                        <x-button class="ml-3 shadow-md bg-blue-800 h-10"> {{ __('Create New User') }} </x-button>
                    @endif
                </div>
...
{{--                Adding Tailwind Alpine Tag Multi Select https://codepen.io/atomgiant/pen/QWjWgKz --}}
                ...
                <section class="border rounded overflow-hidden flex p-6 justify-between">
                    <div class="flex flex-wrap items-start justify-center p-0 w-2/5 bg-white text-white shadow-md">
                        ...
                        <div class="w-full text-black p-2">
                            <input type="text" class="border rounded-lg w-2/3 h-9 shadow-md">
                        </div>
                    ...
                    <div class="flex flex-wrap items-start justify-start p-0 w-2/5 bg-white shadow-md">
                        ...
                        <div class="w-full bg-gray-50 text-black shadow-md p-2">
                            <label class="block">
                                <select class="form-multiselect block h-50 w-full mt-1" multiple>
                                @foreach($indication as $indications)
                                    <option value="{{$indications->id}}">{{$indications->name}}</option>
                                @endforeach
                                </select>
